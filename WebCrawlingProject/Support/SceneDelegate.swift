...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func sceneWillEnterForeground(_ scene: UIScene) {
        ...
        //self.notification()
        SchedulingService.shared.scheduleAppRefresh() // 포그라운드에도 작업 추가
        notification()
    }
...
//        let current_date_string = formatter.string(from: Date())
        ...
        guard let saveCurrentDate = UserDefaults.standard.string(forKey: "saveCurrentDate") else { return }
        
        let docRef = db.collection("Menu").document(saveCurrentDate)
